...
<title>场地使用打卡 - 齐齐哈尔大学体育馆网上系统后台</title>
      ...
      <c:if test="${requestScope.bookList!=null}">
        <table class="ground-list" cellspacing="2px">
          ...
          <c:forEach items="${requestScope.bookList}" var="list">
            <tr>
              <td>${list.gId}</td>
              <td>${list.bStartTime}</td>
              <td>${list.bEndTime}</td>
              <td>${list.bFee}</td>
              <td>${list.bStatus}</td>
              <td class="align-center"><a href="index.html?action=use&uid=${list.uId}&gid=${list.gId}&bid=${list.bId}">使用</a></td>
            </tr>
          </c:forEach>
        </table>
      </c:if>
