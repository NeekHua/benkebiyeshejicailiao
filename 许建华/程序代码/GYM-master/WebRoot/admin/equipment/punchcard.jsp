...
<title>器材租借打卡 - 齐齐哈尔大学体育馆网上系统后台</title>
    ...
    <c:if test="${requestScope.rentList!=null}">
      <table id="book-list" cellspacing="2px">
        ...
        <c:forEach items="${requestScope.rentList}" var="list">
          <tr>
            <td>${list.eId}</td>
            <td>${list.borrowBegin}</td>
            <td>${list.borrowEnd}</td>
            <td>${list.borrowFee}</td>
            <td>${list.status}</td>
            <td class="align-center"><a href="index.html?action=rent&uid=${list.uId}&eid=${list.eId}&erentid=${list.eRentId}">租借</a> <a href="index.html?action=return&uid=${list.uId}&eid=${list.eId}&erentid=${list.eRentId}">归还</a></td>
          </tr>
        </c:forEach>
      </table>
    </c:if>
