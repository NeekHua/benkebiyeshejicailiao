...
<title>赛事管理 - 齐齐哈尔大学体育馆网上系统</title>
...
-->
    <table class="match-list" cellspacing="2px">
      ...
      <c:forEach items="${requestScope.matchList}" var="list">
        <tr>
          <td><a href="../../match/index.html?mid=${list.mId}">${list.mName}</a></td>
          <td>${list.aId}</td>
          <td>${list.mDate}</td>
          <td><a href="index.html?action=delete&mid=${list.mId}" onclick="return confirm('确认删除该赛事？');">删除</a></td>
        </tr>
      </c:forEach>
    </table>
  </div>
